...
for model in models:
	for i_try in range(n_times):
		...
		# for id in range(N):
		# 	for key in plaintext_list[0].keys():
		# 		temp = torch.mul(torch.flatten(plaintext_list[id][key]), 0.3)
		# 		plaintext_list[id][key] = ts.plain_tensor(temp)
		# print("Done Prep")
	...
	print(str(model_counter)+" Total time: {}".format(t_cipher))
	with open('nvidia_results.txt', 'a') as f:
		f.write("Model #"+str(model_counter)+"\n")
		f.write("Init Time: {}".format(t_init)+"\n")
		f.write("Encryption Time: {}".format(t_enc)+"\n")
		f.write("Secure Agg Time: {}".format(t_agg)+"\n")
		f.write("Decryption Time: {}".format(t_dec)+"\n")
		f.write(" Total time: {}".format(t_cipher)+"\n")
	model_counter += 1
