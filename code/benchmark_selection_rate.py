...
def plain_aggregate(global_model, client_models, sel_rate):
...
for i_try in range(n_times):
	...
	plain_aggregate(global_model, client_models, sel_rate)
	...
	for key in learner_data_layer[0].keys():
		for id in range(N):
			...
			cutoff_index = round(len(learner_data_layer[id][key]) * sel_rate)
			enc_learner_layer[id][key] = FHE_helper.encrypt(learner_data_layer[id][key][:cutoff_index])
	...
	for key in model.state_dict().keys():
		model_size[key] = round(torch.flatten(model.state_dict()[key]).numpy().size * sel_rate)
